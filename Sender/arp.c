...
void send_ARP(void)
{
	...
	memcpy(ETH_Frame -> h_dest, broadcast_mac, ETH_ALEN);
...
void receive_ARP(void)
{
	...
	while (1)
	{
		...
		struct arphdr *ARP_Frame = (struct arphdr *)(recv_buffer + sizeof(struct ethhdr));

		printf("============================\n");
		printf("Address Resolution Protocol:\n");
		printf("Sender MAC address: ");
		Print_MAC(ARP_Frame -> smac);
		printf("Sender IP address: ");
		Print_IP(ARP_Frame -> sip);
		printf("Target MAC address: ");
		Print_MAC(ARP_Frame -> tmac);
		printf("Target IP address: ");
		Print_IP(ARP_Frame -> tip);
		printf("\n");
...
int main(int argc, char *argv[]) //interface, source_mac, source_ip, target_ip
{
