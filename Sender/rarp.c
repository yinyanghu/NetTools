...
#define RARP_SIZE	sizeof(struct arphdr)+sizeof(struct ethhdr)
...
void send_RARP(void)
{
	...
	memcpy(ETH_Frame -> h_dest, target_mac, ETH_ALEN);
	memcpy(ETH_Frame -> h_source, source_mac, ETH_ALEN);
	ETH_Frame -> h_proto = htons(ETH_P_RARP);
	
	struct arphdr *RARP_Frame = (struct arphdr *)(send_buffer + sizeof(struct ethhdr));
	RARP_Frame -> hardware = htons(ARPHRD_ETHER);
	RARP_Frame -> proto = htons(ETH_P_IP);
	RARP_Frame -> hardlen = 6;
	RARP_Frame -> pln = 4;
	RARP_Frame -> op = htons(ARPOP_RREQUEST);

	memcpy(RARP_Frame -> smac, source_mac, ETH_ALEN);
	memcpy(RARP_Frame -> sip, source_ip, 4);
	memcpy(RARP_Frame -> tmac, target_mac, ETH_ALEN);


	if (sendto(sock_fd, send_buffer, RARP_SIZE, 0, (struct sockaddr *)&interface, sizeof(interface)) < 0)
	{
...
void receive_RARP(void)
{
	...
	while (1)
	{
		...
		struct arphdr *RARP_Frame = (struct arphdr *)(recv_buffer + sizeof(struct ethhdr));

		printf("============================\n");
		printf("Address Resolution Protocol:\n");
		printf("Sender MAC address: ");
		Print_MAC(RARP_Frame -> smac);
		printf("Sender IP address: ");
		Print_IP(RARP_Frame -> sip);
		printf("Target MAC address: ");
		Print_MAC(RARP_Frame -> tmac);
		printf("Target IP address: ");
		Print_IP(RARP_Frame -> tip);
		printf("\n");
...
int main(int argc, char *argv[]) //interface, source_mac, source_ip, target_mac
{
	if ((sock_fd = socket(PF_PACKET, SOCK_RAW, htons(ETH_P_RARP))) < 0)
	{
	...
	Mac2Bin(argv[2], source_mac);
	Ip2Bin(argv[3], source_ip);
